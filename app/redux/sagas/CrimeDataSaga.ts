...
function* fetchCrimeDataSaga(): Generator<
  | ReturnType<typeof call>
  | PutEffect<{ payload: CrimeData[]; type: "CrimeData/fetchCrimeSuccess" }>
  | PutEffect<{ payload: string; type: "CrimeData/fetchCrimeFailure" }>,
  void,
  AxiosResponse<CrimeData[]>
>
{
  try {
    const response: AxiosResponse<CrimeData[]> = yield call(fetchCrimeDataApi);
